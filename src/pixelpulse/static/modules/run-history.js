...
function _render() {
  ...
  container.innerHTML = runs.map(run => {
    const statusClass = `run-history__status--${run.status || "unknown"}`;
    ...
    return `<div class="run-history__item" data-run-id="${_esc(run.id)}">
      <div class="run-history__top">
        <span class="run-history__name">${_esc(run.name || "Unnamed Run")}</span>
        <span class="run-history__status ${statusClass}">${_esc(run.status)}</span>
      </div>
      <div class="run-history__meta">
        <span>${time}</span>
      ...
      <div class="run-history__actions">
        <button class="btn run-history__btn" data-action="replay" title="Replay">&#x25B6; Replay</button>
        <button class="btn run-history__btn" data-action="export" title="Export JSON">&#x2B73; Export</button>
        <button class="btn run-history__btn" data-action="delete" title="Delete">&#x2715;</button>
      </div>
    </div>`;
  }).join("");

  // Bind click handlers
  container.querySelectorAll(".run-history__btn").forEach(btn => {
  ...
  container.querySelectorAll(".run-history__item").forEach(item => {
